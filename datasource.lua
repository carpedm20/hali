...
function WordDataset:__init(config)
  ...
  self.sets = {}
  for _, ftype in iparis({'train','test','valid'}) do
    if config.filename[ftype] then
      self.filename = paths.concat(self.root, config.filename[ftype])
      self.sets['train'] = torch.load(self.filename)
    end
  end
  collectgarbage()
end
